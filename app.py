...
def load_users():
    if not os.path.exists(USERS_FILE):
        default = {"admin": {"password": "adminshihab123", "role": "admin"}}
        save_users(default)
        return default
    with open(USERS_FILE, 'r', encoding='utf-8') as f:
        data = json.load(f)
    if 'admin' not in data:
        data['admin'] = {"password": "adminshihab123", "role": "admin"}
        save_users(data)
    return data
...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# SHIHAB HOSTING - Default Bot
import time

print("=" * 40)
print("Bot is running on SHIHAB HOSTING")
...
def api_create_server():
    ...
    if not username:
        username = f"SHIHAB_CODEX{random.randint(10000, 99999)}"
